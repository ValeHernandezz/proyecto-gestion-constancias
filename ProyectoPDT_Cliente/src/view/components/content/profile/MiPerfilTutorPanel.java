...
public class MiPerfilTutorPanel extends JPanel {
	...
	// Extras
	private Tutor oUsuarioLogueado;
	private String departamento = null;

	// Método para cargar datos del usuario en los textField y comboBoxes
	private void cargarDatos() {
		oUsuarioLogueado = Buscar.tutorFiltro(Fabrica.getoUsuarioLogueado().getDocumento().toString(), "Documento")
				.get(0);
		...
		comboBoxArea.setSelectedItem(oUsuarioLogueado.getArea().getDescripcion());
		comboBoxRol.setSelectedItem(oUsuarioLogueado.getUsuario().getRol().getDescripcion());
	}

	public MiPerfilTutorPanel(MostrarPanel panel) {
		setLayout(null);
		...
		comboBoxArea.setBounds(227, 202, 151, 21);
		...
		comboBoxRol.setBounds(416, 202, 151, 21);
		...
		buttonModificar.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				int comboIndexGeneración = comboBoxArea.getSelectedIndex();
				int comboIndexSemestre = comboBoxRol.getSelectedIndex();
				...
				try {
					...
					Rol rol = ServiceRol.listarRolesFiltro("Tutor").get(0);
					...
					Area area = ServiceArea.listarAreasFiltro(comboBoxArea.getSelectedItem().toString()).get(0);
					...
					var datosAntiguos = Buscar.tutorFiltro(String.valueOf(cedula), "Documento").get(0);
					...
					Tutor oTutorActualizado = new Tutor(area, oUsuarioActualizado);
					oTutorActualizado.setIdTutor(datosAntiguos.getIdTutor());
					// Actualizo los datos
					...
					var oUsuarioEditado = Actualizar.usuario(oUsuarioActualizado, oTutorActualizado);
